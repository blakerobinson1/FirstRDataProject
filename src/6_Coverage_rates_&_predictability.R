...
shannon_all <- left_join(covrates, defEPA, by = c('defteam')) |> 
...
shannon_all |> 
  ...
  labs(title = "Coverage predictability and defensive pass efficiency allowd for NFL defenses, 2023",
       caption = "By Blake Robinson | @blakerbsn | data: nflreadr",
       subtitle = "Predictability only looking at coverage schemes. Defensive fronts, formations, personnel usage not included") +
  theme(axis.title = element_text(size = 18)) + xlab("Coverage scheme shannon entropy") + ylab('EPA/attempt allowed') +
  theme(panel_grid.minor = element_blank()) +
...
KCtbl <- covratesEPA |> 
  ungroup() |> 
  filter(defteam == 'KC') |> 
  select(team_wordmark, defense_coverage_type, plays, EPAplay, success_rate, YPP) |> 
  arrange(-plays) |> 
  gt() |> 
  gt_img_rows(columns = team_wordmark) |> 
  fmt_number(columns = c(EPAplay, YPP), decimals = 2) |> 
  fmt_percent(columns = success_rate, decimals = 2) |> 
  opt_row_striping() |> 
  gt_theme_538() |> 
